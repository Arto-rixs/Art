            ...
            if (!command.empty()) {
                ...
                if (socket.send(message.c_str(), message.size()) != sf::Socket::Done) {
                    ...
                    if (socket.receive(buffer, sizeof(buffer), received) == sf::Socket::Done) {
                        ...
                        // Добавляем проверку на пустоту полученных данных
                        if (!receivedData.empty()) {
                            std::lock_guard<std::mutex> lock(dataMutex);
                            responses.push_back(utf8_to_wstring(receivedData));
                        } else {
                            logMessage("Received empty data");
                        }
                    } else {
                        logMessage("Failed to receive data from server");
                    }
                }
...
// Функция для обработки и обновления свечек
void updateCandleData(std::vector<Candle>& sampleData, const json& tradeData) {
    int year = tradeData["datetime"]["year"];
    int month = tradeData["datetime"]["month"];
    int day = tradeData["datetime"]["day"];
    float price = tradeData["price"];

    // Формируем дату в виде строки
    std::string date = std::to_string(year) + "-" +
                       (month < 10 ? "0" : "") + std::to_string(month) + "-" +
                       (day < 10 ? "0" : "") + std::to_string(day);

    // Поиск свечи с указанной датой
    auto it = std::find_if(sampleData.begin(), sampleData.end(), [&date](const Candle& candle) {
        return candle.date == date;
    });

    if (it != sampleData.end()) {
        // Обновление существующей свечи
        it->close = price;
        if (price > it->high) it->high = price;
        if (price < it->low) it->low = price;
    } else {
        // Создание новой свечи
        sampleData.push_back({date, price, price, price, price});
    }
}
...
void drawCandlestickChart(sf::RenderWindow& window, const std::vector<Candle>& data, const sf::Vector2f& position, const sf::Vector2f& size, GraphSettings& settings) {
    ...
    for (size_t i = 0; i < data.size(); ++i) {
        ...
        if (sf::Mouse::getPosition(window).x >= x && sf::Mouse::getPosition(window).x <= x + xStep) {
            ...
            hoverText.setString("Date: " + data[i].date + "\nOpen: " + std::to_string(data[i].open) + "\nClose: " + std::to_string(data[i].high) + "\nHigh: " + std::to_string(data[i].low) + "\nLow: " + std::to_string(data[i].close));
        }
    }
...
std::vector<Candle> sampleData = {
...

    {
        std::lock_guard<std::mutex> lock(dataMutex);
        if (!responses.empty()) {
            std::wstring response = responses.back();
            responses.clear();
            
            std::string responseStr = wstringToString(response);
            logMessage("Processing response: " + responseStr);

            // Проверка на пустоту данных перед парсингом
            if (!responseStr.empty() && responseStr != "[]") {
                try {
                    auto jsonData = json::parse(responseStr);
                    logMessage("JSON parsed successfully");

                    // Обновляем свечные данные с полученными новыми данными
                    updateCandleData(sampleData, jsonData);
                    logMessage("Candle data updated successfully");
                } catch (const std::exception& e) {
                    logMessage("Error parsing JSON: " + std::string(e.what()));
                } catch (...) {
                    logMessage("Unknown error occurred while parsing JSON");
                }
            } else {
                logMessage("Empty or invalid JSON response received, skipping processing");
            }
        }
    }
