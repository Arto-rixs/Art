...
void drawCandlestickChart(sf::RenderWindow& window, const std::vector<Candle>& data, const sf::Vector2f& position, const sf::Vector2f& size, float& xOffset, float& scale, tgui::Label::Ptr tooltip) {
    if (data.empty()) return;

    float xStep = size.x / data.size() * scale;
    ...
    for (size_t i = 0; i < data.size(); ++i) {
        float x = position.x + (i + xOffset) * xStep;
        ...
        if (body.getGlobalBounds().contains(window.mapPixelToCoords(sf::Mouse::getPosition(window)))) {
            tooltip->setVisible(true);
            tooltip->setText("Date: " + data[i].date + "\nOpen: " + std::to_string(data[i].open) + "\nClose: " + std::to_string(data[i].close) + "\nHigh: " + std::to_string(data[i].high) + "\nLow: " + std::to_string(data[i].low));
            tooltip->setPosition(sf::Mouse::getPosition(window).x, sf::Mouse::getPosition(window).y);
        }
    }
}
...
int main() {
    ...
    std::vector<Candle> sampleData = {
        {78.0f, 80.0f, 81.0f, 77.0f, "2024-01-01"},
        {80.0f, 82.0f, 83.0f, 79.0f, "2024-01-02"},
        {82.0f, 81.0f, 84.0f, 80.0f, "2024-01-03"},
    ...
    tooltip->getRenderer()->setBackgroundColor(sf::Color::Black);
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) {
                ...
                xOffset += event.mouseMove.x - sf::Mouse::getPosition(window).x;
                sf::Mouse::setPosition(sf::Mouse::getPosition(window), window);
            }
        ...
        drawCandlestickChart(window, sampleData, sf::Vector2f(80, 70), sf::Vector2f(800, 600), xOffset, scale, tooltip);
