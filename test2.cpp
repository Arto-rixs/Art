...
int main() {
    ...
    matrix<float> sample;
    sample.set_size(1, 25 * 25);  // Пример размера игрового поля
    for (int i = 0; i < 25 * 25; ++i) {
        sample(0, i) = static_cast<float>(std::rand()) / RAND_MAX;  // Пример случайного заполнения данных
    }

    auto output = net(sample); // Получаем результат типа output_label_type
    Setup();
    while (true) {
        Draw();
        Input();
